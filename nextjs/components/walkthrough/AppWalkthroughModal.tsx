...
import {
  useWalkthroughStore,
  TOTAL_SHOWCASE_STEPS,
} from "@/lib/store/walkthroughStore";

// ─── Step 1 Preview: Real Animated Balance Card ──────────────────────────────
function BalanceCardPreview() {
  return (
    <div className="w-full bg-[var(--bg-card)] rounded-2xl p-4 sm:p-5 border border-[var(--border-color)] shadow-lg shadow-indigo-500/5 space-y-3">
        ...
        <div className="flex flex-wrap gap-1.5">
          {[
            ...
            <motion.span
              ...
              className={`text-xs px-2.5 py-1 rounded-xl font-medium flex items-center gap-1 cursor-pointer ${
                cat.active
...
function PrivacyPreview() {
  return (
    <div className="w-full bg-[var(--bg-card)] rounded-2xl p-4 sm:p-5 border border-[var(--border-color)] shadow-lg shadow-indigo-500/5 space-y-4 text-center">
      <motion.div
...
export function AppWalkthroughModal() {
  ...
  useEffect(() => {
    if (!isOpen || mode !== "showcase") return;
  ...
  }, [isOpen, mode, nextStep, prevStep, closeWalkthrough]);

  if (!mounted || !isOpen || mode !== "showcase") return null;

  const current = STEPS[currentStep] || STEPS[0];
  const isLastStep = currentStep === TOTAL_SHOWCASE_STEPS - 1;

  const handleStartSpotlightTour = () => {
    openWalkthrough("spotlight");
  };
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-4 md:p-6 overflow-y-auto">
        ...
        <motion.div
          ...
          className="relative w-full max-w-2xl bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh] z-10"
        >
          {/* Header Bar */}
          <div className="px-5 py-4 border-b border-[var(--border-color)] bg-[var(--bg-card)] flex items-center justify-between flex-shrink-0">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-xl overflow-hidden bg-white/5 border border-[var(--border-color)] flex items-center justify-center p-0.5 shadow-sm">
                <Image
              ...
              <div>
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] px-1.5 py-0.2 rounded-full bg-[#6366F1]/10 text-[#6366F1] font-bold">
                    Step {currentStep + 1} of {TOTAL_SHOWCASE_STEPS}
                  </span>
                </div>
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={closeWalkthrough}
                className="px-3 py-1.5 rounded-xl text-xs font-semibold text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-card-hover)] transition-all flex items-center gap-1"
          ...
          <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-6">
            ...
            <div className="flex items-center justify-between">
              ...
              {/* Interactive Spotlight Tour Alternative Button */}
              <button
                onClick={handleStartSpotlightTour}
                className="text-xs font-semibold text-[var(--text-secondary)] hover:text-[#6366F1] flex items-center gap-1.5 transition-colors group"
                title="Highlight actual buttons on the real dashboard"
              >
                <Compass size={14} className="group-hover:rotate-45 transition-transform" />
                <span>Switch to Live UI Spotlight Tour</span>
              </button>
            </div>
            ...
            {/* Pro-Tips Checklist */}
            <div className="bg-[var(--bg-card)] rounded-2xl p-4 border border-[var(--border-color)] space-y-2.5">
          ...
          {/* Footer Controls */}
          <div className="px-5 py-4 border-t border-[var(--border-color)] bg-[var(--bg-card)] flex flex-col sm:flex-row items-center justify-between gap-3 flex-shrink-0">
            {/* Show on Sign-in Checkbox Toggle */}
            <label className="flex items-center gap-2 cursor-pointer select-none text-xs text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors">
              <input
                ...
                className="w-4 h-4 rounded border-[var(--border-color)] text-[#6366F1] focus:ring-[#6366F1] cursor-pointer accent-[#6366F1]"
              />
              <span>Show tutorial every time at sign-in</span>
            </label>
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto justify-between sm:justify-end">
              ...
              <button
                ...
                className="px-3.5 py-2 rounded-xl text-xs font-bold text-[var(--text-secondary)] border border-[var(--border-color)] hover:bg-[var(--bg-card-hover)] hover:text-[var(--text-primary)] transition-all disabled:opacity-30 disabled:cursor-not-allowed flex items-center gap-1"
              >
                <ChevronLeft size={16} />
                <span>Back</span>
              </button>
              ...
              <button
                onClick={nextStep}
                className="px-4 py-2 rounded-xl text-xs font-bold bg-[#6366F1] hover:bg-[#5558E6] text-white shadow-md shadow-indigo-500/20 transition-all flex items-center gap-1.5 active:scale-95"
              >
                <span>{isLastStep ? "Start Tracking 🚀" : "Next"}</span>
