...
export default function BudgetsPage() {
  ...
  const { budgets, goals, deleteBudget, deleteGoal, addFundsToGoal } = useBudgetStore();
  ...
  return (
    <div className="min-h-full bg-[var(--bg-primary)]">
      ...
      {/* Main Container: 1 col on mobile, 2 col on laptop/PC */}
      <div className="px-4 pb-8 pt-4 md:py-6 space-y-6 md:space-y-0">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
          {/* ── Budgets Section (Exact same cards, buttons, and fonts) ── */}
          <section>
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <span
                              className={`text-xs font-semibold ${
                                over ? "text-[#EF4444]" : "text-[var(--text-secondary)]"
                              }`}
                            >
                              {over
                                ? `${symbol}${(budget.spent - budget.limit).toLocaleString()} over`
                                : `${symbol}${(budget.limit - budget.spent).toLocaleString()} left`}
                            </span>
                          </div>
                        </div>
                      </div>
          ...
          {/* ── Goals Section (Exact same cards, buttons, and fonts) ── */}
          <section>
            <div className="flex items-center justify-between mb-3">
            ...
            {goals.length === 0 ? (
              <div className="bg-[var(--bg-card)] rounded-2xl border border-[var(--border-color)] p-8 text-center">
                ...
                <p className="text-sm font-semibold text-[var(--text-primary)]">No savings goals yet</p>
                <p className="text-xs text-[var(--text-secondary)] mt-1">Tap + Add to start saving for a goal</p>
              </div>
            ) : (
              <div className="space-y-3">
                {goals.map((goal) => {
